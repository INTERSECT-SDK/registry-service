...
async def microservice_user_page(
    ...
    username = user[0]
    invalid_service = request.cookies.get(CTX_INVALID_SERVICE, '')
    misc_error = request.cookies.get(CTX_SERVER_ERROR_SERVICE, '')
    ...
    response = TEMPLATES.TemplateResponse(
        ...
        context={
            ...
            'server_error_service': misc_error,
        },
        headers=headers,
    )
    if invalid_service:
        response.delete_cookie(CTX_INVALID_SERVICE, secure=True, httponly=True, samesite='strict')
    elif misc_error:
        response.delete_cookie(
            CTX_SERVER_ERROR_SERVICE, secure=True, httponly=True, samesite='strict'
        )
...
def _add_new_service_error(
    ...
    if is_htmx_request(request):
        error_key = 'server_error_service' if server_fault else 'invalid_service'
        # Javascript is enabled, so we can return an HTML partial
        ...
        return TEMPLATES.TemplateResponse(
            ...
            context={error_key: service_name},
            headers={
                'HX-Reswap': 'innerHTML',
    ...
    response = RedirectResponse('/', status_code=302)
    response.set_cookie(
        CTX_SERVER_ERROR_SERVICE if server_fault else CTX_INVALID_SERVICE,
        service_name,
        secure=True,
        httponly=True,
        samesite='strict',
