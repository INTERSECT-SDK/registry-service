...
def _get_html_security_headers(nonce: str) -> dict[str, str]:
    ...
    return {
        ...
        'Content-Security-Policy': f"default-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'; connect-src 'self'; script-src 'nonce-{nonce}'; style-src 'nonce-{nonce}'; style-src-elem 'nonce-{nonce}'; img-src 'self'",
    }
